...
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│  Mode Selection │───▶│  Party Lobby    │───▶│  IGL/Anchor     │
│  (5v5 card)     │    │  (1-4 players)  │    │  Pre-Selection  │
└─────────────────┘    └─────────────────┘    └────────┬────────┘
                                                       │
┌─────────────────┐    ┌─────────────────┐    ┌────────▼────────┐
│  IGL Confirm    │◀───│  Team Formed    │◀───│  Queue          │
│  (full team)    │    │  (5 assembled)  │    │  (fill spots)   │
└────────┬────────┘    └─────────────────┘    └─────────────────┘
         │
┌────────▼────────┐    ┌─────────────────┐
│  Pre-Match      │───▶│  Active Match   │
│  Strategy       │    │                 │
└─────────────────┘    └─────────────────┘
```
...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│  Mode Selection │───▶│  "Find Team"    │───▶│  Queue (solo)   │
│  (5v5, no party)│    │  Entry Modal    │    │  (building team)│
└─────────────────┘    └─────────────────┘    └────────┬────────┘
                                                       │
┌─────────────────┐    ┌─────────────────┐    ┌────────▼────────┐
│  Pre-Match      │◀───│  IGL Vote/Auto  │◀───│  Team Formed    │
│  Strategy       │    │  Selection      │    │  (5 matched)    │
└────────┬────────┘    └─────────────────┘    └─────────────────┘
         │
┌────────▼────────┐
│  Active Match   │
...
These components handle the selection of In-Game Leader and Anchor roles before queuing.

#### Component: `IGLSelectionModal`

**Purpose:** Allow party to choose IGL and Anchor before queuing.

**Location:** `src/components/arena/igl-selection-modal.tsx` (new)

**Trigger Conditions:**
- When full party and leader clicks "Queue"
- When solo/half-party forms complete team in queue
...
When queuing solo or with partial party for team mode:
...
| Step | Action |
|------|--------|
| 1. Match Found | System forms complete 5-player team |
| 2. IGL Selection | Quick selection: 5-second timer |
| 3. Auto-Assign Priority | 1) Volunteers 2) Highest ELO 3) Largest party leader |
| 4. Anchor Selection | Same priority order after IGL assigned |
| 5. Confirmation | All players see roles, 3-second countdown to match |

**Quick Selection UI (Solo Queue):**

```
┌────────────────────────────────────────────────────────────┐
│  TEAM FORMED!                                  Time: 0:05  │
├────────────────────────────────────────────────────────────┤
│                                                            │
│  Your Team:                                                │
│  ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐             │
...
│  └──────┘ └──────┘ └──────┘ └──────┘ └──────┘             │
│                                                            │
│  Assigning roles based on preferences...                  │
│  IGL: Player2 (volunteered)                               │
│  Anchor: YOU (highest ELO volunteer)                      │
│                                                            │
│  [Starting match in 3...]                                 │
└────────────────────────────────────────────────────────────┘
```
...
|-------|-----------|---------|
| Building Team | seeking > 0 | "Finding X more players..." |
| Team Ready | seeking === 0 | "Team ready! Finding opponents..." |
| Match Found | - | "MATCH FOUND! Confirming players..." |
| Confirmed | - | Transition to pre-match |
...
│  ████████████░░░░░░░░░░░░░░░░░░░░░░░░░░░░                         │
│                                                                    │
│  ⏱ Time in queue: 0:42         Est. wait: ~1:30                  │
│                                                                    │
│                        [CANCEL QUEUE]                              │
└────────────────────────────────────────────────────────────────────┘
```
