...
export async function kickMember(
    ...
    leaderId: string,
    targetUserId: string
): Promise<{ success: boolean; kickedName?: string; queueCancelled?: boolean; error?: string }> {
    const redis = await getRedis();
    if (!redis) {
        return { success: false, error: 'Redis unavailable' };
    }
    ...
    try {
        ...
        return { success: true, kickedName: targetMember.odUserName, queueCancelled };
...
export async function toggleReady(
    partyId: string,
    userId: string
): Promise<{ success: boolean; isReady?: boolean; queueCancelled?: boolean; error?: string }> {
    const redis = await getRedis();
    if (!redis) {
        return { success: false, error: 'Redis unavailable' };
    }
    ...
    try {
        ...
        return { success: true, isReady: newReadyState, queueCancelled };
...
export async function acceptInvite(
    ...
    try {
        const inviteData = await redis.hget(KEYS.invites(partyId), userId);
        if (!inviteData) {
        ...
        if (invite.expiresAt < Date.now()) {
            await redis.hdel(KEYS.invites(partyId), userId);
        ...
        // Join the party
        return await joinParty(partyId, userId, userName, userData);
