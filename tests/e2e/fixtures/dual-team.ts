...
/**
 * Navigate all players on a team to a specific URL with retry logic
 */
export async function navigateTeam(team: TeamFixture, url: string): Promise<void> {
    const navigateWithRetry = async (page: Page, targetUrl: string, retries = 3): Promise<void> => {
    ...
    await Promise.all(team.allPlayers.map(p => navigateWithRetry(p.page, url)));
}

/**
 * Navigate ALL 10 players to a specific URL with retry logic
 */
export async function navigateAllPlayers(dualTeam: DualTeamFixture, url: string): Promise<void> {
    ...
    await Promise.all(dualTeam.allPlayers.map(p => navigateWithRetry(p.page, url)));
}
