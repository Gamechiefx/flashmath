...
async function startTeamQueue(team: TeamFixture, teamLabel: string): Promise<void> {
    ...
    // Click Find Match button (which navigates to queue page)
    const findMatchBtn = leader.page.locator('[data-testid="find-match-button"]')
        .or(leader.page.getByRole('button', { name: /start team queue|find match|queue|search/i }));
    
    const canQueue = await findMatchBtn.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    if (canQueue) {
        ...
        try {
            await leader.page.waitForURL(/\/arena\/teams\/queue/, { timeout: 10000 });
        ...
        console.log(`[${teamLabel}] Available buttons: ${buttons.slice(0, 8).join(', ')}`);
    }
}
