...
export class SocialPage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        // Party actions
        this.createPartyButton = page.locator('[data-testid="create-party-social"]');
    ...
    async openPanel(): Promise<void> {
        ...
        await this.socialFab.waitFor({ state: 'visible', timeout: 10000 });
        
        // Use JavaScript click for fixed-positioned elements (bypasses viewport checks)
        await this.socialFab.evaluate((el: HTMLElement) => el.click());
        await this.page.waitForTimeout(800); // Wait for slide-in animation
        
        // Verify panel opened
        const opened = await this.isPanelOpen();
        if (!opened) {
            // Fallback: try force click
            await this.socialFab.click({ force: true }).catch(() => {});
